...
class ProfissionaisService {
  ...
  async listarProfissionais(): Promise<Profissional[]> {
    try {
      ...
      const profissionais = await apiClient.get<Profissional[]>('/professionals');
  ...
  async listarProfissionaisIndicados(): Promise<ProfissionalIndicado[]> {
    try {
      ...
      const response =  await apiClient.get<ProfissionalIndicado[]>('/recommended-professionals');
  ...
  async alterarStatusProfissionalIndicado(id: string): Promise<void> {
    try {
      ...
      const response = await apiClient.patch(`/recommended-professionals/toggle-status/${id}`, {});
